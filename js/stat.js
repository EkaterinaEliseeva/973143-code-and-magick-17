...
window.renderStatistics = function (ctx, names, times) {
  ...
  for (var i = 0; i < names.length; i++) {
    ...
    renderText(ctx, (CLOUD_X + 3 * GAP) + (BAR_GAP + TEXT_WIDTH) * i, CLOUD_Y + 2 * FONT_SIZE + 5 * GAP + BAR_HEIGHT - (BAR_HEIGHT * Math.floor(times[i]) / Math.floor(maxTime)), Math.floor(times[i]), 'rgb(0, 0, 0)');
    if (names[i] === 'Вы') {
      ctx.fillStyle = 'rgba(255, 0, 0, 1)';
    } else {
      ctx.fillStyle = 'hsl(225,' + (Math.random() * 100) + '%, 50%)';
    }
    ctx.fillRect((CLOUD_X + 3 * GAP) + (BAR_GAP + TEXT_WIDTH) * i, CLOUD_Y + 2 * FONT_SIZE + 6 * GAP + BAR_HEIGHT - (BAR_HEIGHT * Math.floor(times[i]) / Math.floor(maxTime)), TEXT_WIDTH, BAR_HEIGHT * Math.floor(times[i]) / Math.floor(maxTime));
  }
};
